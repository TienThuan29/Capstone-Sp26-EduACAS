...
type ProblemFormProps = {
  ...
  extractedContentHelperText?: string;
};
...
export function ProblemForm({
  ...
  submitButtonLabel,
  submitButtonIcon,
  extractedContentHelperText = "Review and edit the extracted content before saving",
}: ProblemFormProps) {
  ...
  const [modeState, setModeState] = useState<ProblemMode>("MANUAL");
  ...
  useEffect(() => {
    ...
    if (!id) {
      setLoading(false);
      toast.showError("Invalid problem");
    ...
    const loadProblem = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          // Manual entry uses fileName "manual_problem.txt" or empty; treat as MANUAL for editor display
          const isManualEntry =
            !data.fileName || data.fileName === "manual_problem.txt";
          setModeState(
            isManualEntry ? PROBLEM_MODE.MANUAL : PROBLEM_MODE.FROM_FILE,
          );
          setShowEditor(false);
          // Load content into editor whenever we have content (manual or extracted)
          if (data.content) {
    ...
    loadProblem();
  }, [mode, id, getProblemById, toast, router]);

  useEffect(() => {
    if (editor && pendingEditorContent) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modeState === PROBLEM_MODE.MANUAL) {
        if (editor) {
          const html = editor.getHTML();
          finalContent = new TurndownService().turndown(html);
        }
        ...
        if (showEditor && editor) {
          finalContent = new TurndownService().turndown(editor.getHTML());
        } else {
          finalContent = "";
        }
      }
      ...
      const err = error as { response?: { data?: { message?: string } } };
      toast.showError(
        err.response?.data?.message ?? `Failed to ${mode} problem`,
      );
    } finally {
      setSubmitting(false);
    }
        ...
        >
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div
                    onClick={() => {
                      setModeState(PROBLEM_MODE.MANUAL);
                      setShowEditor(false);
                    }}
                    className={`cursor-pointer rounded-lg border-2 p-4 transition-all ${
                      modeState === PROBLEM_MODE.MANUAL
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`rounded-full p-2 ${modeState === PROBLEM_MODE.MANUAL ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-500"}`}
                      >
                  ...
                  <div
                    onClick={() => {
                      setModeState(PROBLEM_MODE.FROM_FILE);
                      setShowEditor(false);
                    }}
                    className={`cursor-pointer rounded-lg border-2 p-4 transition-all ${
                      modeState === PROBLEM_MODE.FROM_FILE
                        ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20"
                        : "border-gray-200 hover:border-blue-300 dark:border-gray-700"
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`rounded-full p-2 ${modeState === PROBLEM_MODE.FROM_FILE ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-500"}`}
                      >
            ...
            >
              ...
              >
                Problem Content <span className="text-sm text-red-500">*</span>
              </h2>
              ...
              {modeState === PROBLEM_MODE.MANUAL && (
                <TextEditor
                  editor={editor}
                  isDark={isDark}
                  helperText="Use the toolbar to format your problem description"
                />
              )}
                  ...
                  {showEditor && (
                    <div className="animate-fade-in mt-4">
                      <TextEditor
                        editor={editor}
                        isDark={isDark}
                        label="Edit Extracted Content"
                        helperText={extractedContentHelperText}
                      />
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <Button
                    fullSized
                    color="light"
                    className="mt-3 cursor-pointer border-dashed"
