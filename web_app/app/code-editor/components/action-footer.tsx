...
import { Play, Send, Loader2, CheckCircle, XCircle, Clock, AlertCircle } from 'lucide-react';
...
function StatusBadge({ status }: { status: SubmissionStatus }) {
  ...
  > = {
    ...
    wrong_answer: {
      ...
      className: 'bg-red-600/20 text-red-400 border border-red-600/50',
    },
    tle: {
    ...
    runtime_error: {
      ...
      className: 'bg-orange-600/20 text-orange-400 border border-orange-600/50',
    },
    compilation_error: {
...
export function ActionFooter() {
  const { runCode, submitCode, isRunning, isSubmitting, submissionStatus, testCases } =
    useEditorContext();

  const passedCount = testCases.filter((tc) => tc.status === 'pass').length;
  const totalCount = testCases.length;
  const hasResults = testCases.some((tc) => tc.status !== 'untested');
  ...
  return (
    <div className="flex items-center justify-between border-t border-gray-700 bg-gray-900 px-4 py-3">
      ...
      <div className="flex items-center gap-4">
        ...
        {hasResults && submissionStatus === 'idle' && (
